...
3. **Generated artifacts** — if the repo's `docs/agents/verification.md` lists
   generated artifacts (e.g. OpenAPI specs, protobuf stubs, GraphQL schemas),
   run the validation command it specifies for any artifact implicated by the
   diff. If it fails, run the generation command and stage the result.
   Report mode: just report the check's exit status.
...
9. Output the report using **exactly** this structure — these headings
   ...
   ### Generated artifacts
   - <artifact>: regenerated <path> | clean | n/a
   - Symlinks: <result> | n/a
...
- Do **not** regenerate artifacts by hand — use the generator command from
